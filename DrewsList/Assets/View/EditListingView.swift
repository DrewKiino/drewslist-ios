...
public class EditListingView: UIViewController, UITableViewDataSource, UITableViewDelegate {
  ...
  private var model: EditListingModel { get { return controller.getModel() } }
  ...
  private func setupDataBinding() {
    ...
    model._serverCallbackFromDeleteListing.removeAllListeners()
    model._serverCallbackFromDeleteListing.listen(self) { [weak self] bool in
      // show header buttons
  ...
  public func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
    ...
    case 0:
      if let cell = tableView.dequeueReusableCellWithIdentifier("PaddingCell", forIndexPath: indexPath) as? PaddingCell {
        cell.paddingLabel?.text = "Edit Your Book:"
        cell.hideTopBorder()
    ...
    case 8:
      if let cell = tableView.dequeueReusableCellWithIdentifier("PaddingCell", forIndexPath: indexPath) as? PaddingCell {
        cell.paddingLabel?.text = "Edit Information: "
        cell.alignTextLeft()
        return cell
      }
      break
    ...
    case 10:
      if let cell = tableView.dequeueReusableCellWithIdentifier("InputTextViewCell", forIndexPath: indexPath) as? InputTextViewCell {
        ...
        cell.inputTextView?.placeholder = "You Can Edit Your Notes For Your Book Here Please Keep it Accurate and Clean! "
        cell._isFirstResponder.removeAllListeners()
        cell._isFirstResponder.listen(self) { [weak self] bool in
          if let cell = self?.tableView?.cellForRowAtIndexPath(NSIndexPath(forRow: 8, inSection: 0)) {
            self?.tableView?.setContentOffset(CGPointMake(0, cell.frame.origin.y), animated: true)
    ...
    case 11:
      if let cell = tableView.dequeueReusableCellWithIdentifier("BigButtonCell", forIndexPath: indexPath) as? BigButtonCell {
        cell.buttonLabel?.text = "Save Listing"
