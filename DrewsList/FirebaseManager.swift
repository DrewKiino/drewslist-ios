...
public class FireBaseManager {
  ...
  public class func listen(user_id: String? = nil, completionHandler: (dictionary: [String: AnyObject]) -> Void) {
    if let user_id = user_id ?? AuthenticationManager.currentUser?.uid where listeners[user_id] == nil {
      ...
      dbRef.child("users").child(user_id).observeEventType(.Value, withBlock: { snapshot in
        let dictionary = snapshot.toDictionary()
        // save listener
  ...
  public class func listenOnce(user_id: String? = nil, completionHandler: (dictionary: [String: AnyObject]) -> Void) {
    if let user_id = user_id ?? AuthenticationManager.currentUser?.uid {
...
extension FIRDataSnapshot {
  ...
  public func toDictionary() -> [String: AnyObject] {
    ...
    children.allObjects.forEach { object in
